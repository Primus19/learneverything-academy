  ...
  try {
    ...
    const chapters = chapterFiles.map(file => {
      ...
      return {
        file,
        title: chapterTitle,
        lessons: lessons.length > 0 ? lessons : ['Introduction', 'Core Concepts', 'Practical Examples'],
      };
    });
...
// Get chapter content
export async function getChapterContent(courseSlug: string, chapterIndex: number) {
  try {
    // Convert slug to directory name
